...
public class RPN_Algorithm {
    ...
    RPN_Algorithm(String[] expression) {
        ...
        for (String expression1 : expression) {
            ...
            if (isAnOperator(input)) {
                ...
                Double x = Double.parseDouble((String) stack.pop()); // pop the second element (convert to integer
                                                                     // first)
    ...
    private boolean isAnOperator(String op) { // this function checks if the current element is an operator or not
        return (op.length() == 1 && "DMAS".contains(op)); // this returns a boolean value (true) if the input is correct
                                                          // and is an operator
    } // isAnOperator() ends

    private double evaluate(double x, double y, String op) { // this function performs arithematic operation depending
                                                             // upon the given operator
